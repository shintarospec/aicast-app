...
```
commit e82aaae6
feat: Xサンプルランダム選択機能の実装と関連ドキュメント追加

主な変更:
- 運営指針テキスト一括インポート: アカウントID,ニックネーム から3つランダム選択
- persona_detailed に x_sample_id_1~3, x_sample_name_1~3 カラム追加
- 詳細ペルソナUIに Xサンプル3組の表示・編集機能を実装
- session_state 管理の改善 (parsed_ → edit_ コピーで初期値反映)
- ACCOUNT_GUIDELINES_MAPPING.md: 抽出パターンと対応表を追加
- X_SAMPLE_FEATURE_LOG.md: 作業ログと仕様を記録
- FEATURE_X_SAMPLE_RANDOM_SELECTION.md: 完全な実装ドキュメント
- その他: タスク定義・ドキュメント・バックアップファイル追加

統計: 57ファイル変更、50,772行追加、111行削除
```
...
- **`ACCOUNT_GUIDELINES_MAPPING.md`** - テキスト項目とCSVフィールド対応表、抽出パターン詳細
- **`X_SAMPLE_FEATURE_LOG.md`** - 作業ログと簡易仕様（本ドキュメントの要約版）
...
### 同時に実装された機能
1. **サンプル投稿一括インポート** - CSV形式のサンプル投稿を抽出・プレビュー・DB保存
2. **運営指針テキスト一括インポート** - 運営ミッション/ペルソナ設計意図/コンテンツ戦略/最終目標を自動抽出
3. **主なフォロー対象フィールド** - `main_follow_targets` カラムの追加と抽出対応
4. **ペルソナCSV自動抽出** - 14+フィールドを一括抽出・フォーム自動反映

### 関連コミット
- `c34bee4c` - cleanup: サンプル投稿抽出・インポートのデバッグパネルを削除
- `f249243f` - debug: サンプル投稿抽出・インポートのデバッグ情報を追加
- `6104fe9d` - fix: サンプル投稿抽出・インポートの不具合を修正
- `11f011c5` - improve: サンプル投稿インポート機能を改善
- `1bf1f890` - feat: キャスト運営指針テキスト一括インポート機能を実装
