...
### 承認済み投稿タブ（テーブルデザイン）
- **表示形式**: `st.data_editor`によるテーブルUI（2025年11月8日リニューアル）
- **編集可能列**: 送信日（DateColumn）、送信時刻（TimeColumn）
- **選択機能**: チェックボックス列で予約対象を選択（デフォルト全選択）
- **状態保持**: `st.session_state.approved_selections`で選択状態を永続化
  - 保存ボタンクリック後も選択状態を維持
  - 予約実行後は該当IDをsession_stateから削除
- **操作フロー**:
  1. テーブルで送信日時をインライン編集
  2. 「💾 時刻変更を保存」でDB更新
  3. 不要な投稿は「選択」列のチェックを外す
  4. 「📅 選択した投稿を予約」で選択投稿のみ予約
  5. 予約後、ページリロードで承認一覧から自動削除
- **データ整合性**: `num_rows="fixed"`で行追加・削除禁止、過去日時の設定防止
...
- `FEATURE_UPDATES_2025_10_07.md`：機能追加・改修履歴